...
def main():
    ...
    def process():
        ...
        if presetEntry.get() == 'cc':
            ...
            lati = dmsToDD('37:4:13') 
            long = dmsToDD('119:24:46')             
            loca = EarthLocation(lat=lati*u.deg, lon=long*u.deg, height=0*u.m)
        ...
        for i in range(1,1440):
            ...
            if float(altOne) < 0 and float(altTwo) >= 0:
                ...
                readability.grid(column=0, row=16)  
        ...
        if all(i >= 0 for i in altitude):
            readability = Label(window, text="", font=("Arial Bold", 10)) #Label for txt entry window
            readability.grid(column=0, row=16)  
            nosetLbl = Label(window, text="Source never sets", font=("Arial Bold", 10)) #Label for txt entry window
            nosetLbl.grid(column=0, row=17)      
        elif all(i <= 0 for i in altitude):
            readability = Label(window, text="", font=("Arial Bold", 10)) #Label for txt entry window
            readability.grid(column=0, row=16)              
            noriseLbl = Label(window, text="Source never rises", font=("Arial Bold", 10)) #Label for txt entry window
            noriseLbl.grid(column=0, row=17)    
    ...
    def graph():
        ...
        try:
            plt.axhspan(0, (min(min(sunAlt), min(altitude))) - 1, facecolor='0.2', alpha=0.5)
        except:
            plt.axhspan(0, min(altitude) - 1, facecolor='0.2', alpha=0.5)
